...
$(function () {
    ...
    $('.main_product .tab_menu button').on('click', function () {
        let idx = $(this).index(); // 클릭한 것의 번호를 가져와서 저장함
        console.log(idx);

        $('.main_product .tab_menu button').removeClass('on');
        $(this).addClass('on');

        $('.main_product .tab_content .con').removeClass('on');
        $('.main_product .tab_content .con').eq(idx).addClass('on');
    });
